...
const ExpandableText: FC<Props> = ({ children, className }) => {
  ...
  useEffect(() => {
    ...
    const textRowHeight = Math.round(parseFloat(getComputedStyle(textRef.current).fontSize) * (lineHeight === 'normal' ? 1.2 : parseFloat(lineHeight)));
  ...
  return (
    <div className={classNames(className, styles.wrapper)}>
      <div ref={textRef} className={classNames(styles.text, isExpanded && styles.text_expanded)}>{children}</div>
      {
